...
def estimate_JMIM(x,y):
    model = mifs.MutualInformationFeatureSelector(method='JMIM',categorical=False,n_features = x.shape[1],n_jobs=12)
    model.fit(x,y)
    # while (len(model.mi_) < x.shape[1]): #ensure the proper computation ---ensure that the weight has same length with the data dimension
    #     model = mifs.MutualInformationFeatureSelector(method='JMIM',categorical=False)
    #     model.fit(x,y) #re-fit model
    return model.mi_
...
class Kriging:
    ...
    def get_theta(self,theta0):
        ...
        # we are assuming that COBYLA will be used
        ...
        #             method=self.optimizer,options={"rhobeg": _rhobeg})

        # ,options={"rhobeg": _rhobeg, "tol": 1e-4, "maxiter": limit}
    #    , "tol": 1e-5, "xtol": 1e-4, "maxiter": 1e4, "maxfun": 1e10 

        res = minimize(self.NLL,xk,method="COBYLA",options={'disp':False})
        # res = minimize(self.NLL,xk,method="COBYLA",options={'ftol':1e-20,'disp':False})
