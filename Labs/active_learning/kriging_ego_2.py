...
class Kriging:
    ...
    def __init__(self,x,y,kernel,theta0="",optimizer="COBYLA",optimizer_noise=1.0,eps=1.48e-08,restarts=1,preprocessing="standardize"):
        ...
        self.theta0 = theta0
        self.likelihood = 0
        self.likelihood_threshold = 1 #hack to ensure good values
    ...
    def get_theta(self,theta0):
        ...
        while(self.likelihood < self.likelihood_threshold):
            if self.optimizer == "nelder-mead-c":
                ...
                res = cNM.constrNM(self.NLL,xk,LB,UB,xtol=1e-2,full_output=True)
                theta = res['xopt']

            elif self.optimizer == "COBYLA" :
                theta_bounds = [1e-5,1e5]
                constraints = []
    ...
    def predict_variance(self,testdata):
        ...
        # # Calculate matrix of distances D between samples
        # D, self.ij = cross_distances(self.x_test[0]) #make code efficient by computing this once before optimization
        # # compute the correlation matrix
...
def cross_distances(X):
    ...
    # n_samples , n_features = 1,2 #hardcode test
