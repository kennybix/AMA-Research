...
class Kriging:
    ...
    def __init__(self,x,y,kernels="",theta0="",nu0="",optimizer="CMA-ES",optimizer_noise=0.01,eps=1.48e-08,restarts=3,preprocessing="normalize"):
        ...
        self.likelihood_threshold = 10 #failsafe system
    ...
    def compute_R(self,D,theta,nu):
        ...
        for i in  range(self.Nk):
            ...
            if nu_d == 'inf': #use Gaussian kernel
    ...
    #             # nll = np.inf
    #             nll = 999.0 #fail safe 
    #             # using this makes code stuck too long...why not just slice theta and continue
    ...
    #     # create the variable
    #     nu = np.zeros(self.Nk) # create empty slots
    ...
    #     for step in range(2):
    #         nll_value = self.NLL(hyperparameter)
    #         trial.report(nll_value,step)
    ...
    def objective(self,trial):
        ...
        for i in range(self.Nk): # loop through the dimension
            nu_variable = 'nu' + str(i)
            variable = 'theta' + str(i)
            nu[i] = trial.suggest_categorical(nu_variable, ['inf','0.5', '1.5', '2.5'])
            theta[i] = trial.suggest_float(variable, theta_bound[0], theta_bound[1])
