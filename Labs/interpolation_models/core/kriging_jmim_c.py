...
def sort_mi_values(mi,ranking,reverse=False):
    sorted_mi_values = sort_together([ranking,mi])[1]
    # sorted_mi_values = sort_mi_values.tolist() # converting to list
    if reverse == True:
        sorted_mi_values = sorted_mi_values[::-1]
    else: sorted_mi_values = sorted_mi_values
    return sorted_mi_values

def estimate_JMIM(x,y):
    model = mifs.MutualInformationFeatureSelector(method='JMIM',categorical=False,n_features = x.shape[1])
    model.fit(x,y)
    # mi_values = sort_mi_values(model.mi_,model.ranking_,reverse=True) # isolating this for now
    mi_values = model.mi_
    return mi_values
...
class Kriging:
    ...
    def compute_componentwise_distance(self,D,theta):
        theta_c = theta[0] * np.array(self.coeff_jmim) + theta[1]*np.ones(self.Nk)
    ...
    def get_theta(self,theta0):
        ...
        #     theta = np.copy(res.x) #initialization
            ...
            theta_bounds = [-1e2,1e3]
            constraints = []
            limit, _rhobeg = 10 *(2), 0.5
            # log10t_bounds = np.log10(theta_bounds)
            for ii in range(2):
                constraints.append(lambda t, i=ii: t[i] - theta_bounds[0])
                constraints.append(lambda t, i=ii: theta_bounds[1] - t[i])
        ...
        approximated_theta = theta[0] * np.array(self.coeff_jmim) + theta[1]*np.ones(self.Nk)
        self.info = {'Theta':self.theta,
                        'Approximated Theta':approximated_theta,
    ...
    #     approximated_theta = self.theta[0] * np.array(self.coeff_jmim) + self.theta[1]*np.ones(self.Nk)
    #     self.info = {'Theta':self.theta,
    #                     'Approximated Theta':approximated_theta,
    #                     'JMIM':self.coeff_jmim,
    #                     'Likelihood':self.likelihood}


    def predict(self,testdata):
        # self.get_training_info() #just running this here to get the necessary information
        if(self.Nk == 1):
