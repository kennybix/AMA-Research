...
class active_learn():
    ...
    def loss_func(self,x):
        ...
        if isinstance(y,list):
            ...
            y = y
        L = 0.2*(y-fhat)**2 + 0.8*s2 #modified for maximization
        return -L
    ...
    def get_more_samples(self):
        ...
        while(count<self.max_steps): #stopping criteria
            ...
            self.gp_old = GaussianProcessRegressor(kernel=kernel, alpha=1e-5, n_restarts_optimizer=1)
            # Train the Gaussian process regressor model
            self.gp_old.fit(x_sample, y_sample)
            ...
            while (expected_improvement == 0.0):
                ...
                expected_improvement = self.compute_EI(x_new)
            ...
            # nelder-mead-c
            # Define the Gaussian process regressor model
            self.gp_new = GaussianProcessRegressor(kernel=kernel, alpha=1e-5, n_restarts_optimizer=1)
            # Train the Gaussian process regressor model
            self.gp_new.fit(x_sample, y_sample)
