...
class Kriging:
    ...
    def __init__(self,x,y,kernels,theta0="",weights0="",optimizer="CMA-ES",optimizer_noise=1.0,eps=1.48e-08,restarts=1,preprocessing="standardize",pls_n_comp="", pls_cv=5):
        ...
        if theta0 == "":
            self.theta0 = [0.001] * self.pls_n_comp
        else:
            self.theta0 = theta0
    ...
    def compute_componentwise_distance(self,D,theta):
        ...
        D_corr = np.dot(np.abs(D),np.abs(coeff_pls))
    ...
    def NLL(self, hyperparameter):
        ...
        theta = np.zeros(len(self.theta0)) # initialize theta
        ...
        s = np.sum(weights)
        weights /= s #ensure the weight sums up to 1.0
    ...
    def get_theta(self,hyperparameter):
        ...
        if (self.optimizer=="CMA-ES"):
            ...
            new_bounds = [LB,UB]
            xopts, es = cma.evolution_strategy.fmin_con(self.NLL, xk, 0.1, h=lambda xk: [self.constraint_func(xk)], \
                options={'bounds':new_bounds,'verbose':-9, 'popsize':40},restarts=self.restarts)
    ...
    def train(self):
        theta0 = []
        for i in range(self.pls_n_comp):
            theta0.append(self.theta0[i])
            # theta0.append(self.theta0[i][0])
        hyperparameter = theta0 + self.weights0
    ...
    def predict(self,testdata):
        ...
        r_x = (self.compute_rr(dx,self.theta,self.weights)).reshape(test_size,self.Ns) 
