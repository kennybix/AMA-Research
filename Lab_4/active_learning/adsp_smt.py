...
class active_learn():
    ...
    def compute_EI(self,x):
        ...
        fhat = (self.gp_old)._predict_values(x)
        s2 = (self.gp_old)._predict_variances(x)
    ...
    def loss_func(self,x):
        ...
        fhat = (self.gp_old)._predict_values(x)
        s2 = (self.gp_old)._predict_variances(x)
        # fhat,s2 = (self.gp_old)._predict_values(x.T) #modify predict to return two values just for this process
        fhat = fhat.reshape(len(s2),)
        s2 = s2.reshape(len(s2),)
        y = self.obj_func(x)
        if isinstance(y,list):
    ...
    def get_more_samples(self):
        ...
        while(count<self.max_steps): #stopping criteria
            ...
            self.gp_old = SMT_KRG(corr='squar_exp')
            (self.gp_old).set_training_values(np.array(x_sample),np.array(y_sample))
            (self.gp_old).train()

            # KCORE2.Kriging(x_sample,y_sample,self.kernel,theta0=theta0,optimizer="nelder-mead-c") #initialiize model
            ...
            self.gp_new = SMT_KRG(corr='squar_exp')
            (self.gp_new).set_training_values(np.array(x_sample),np.array(y_sample))
            (self.gp_new).train()
            ...
            self.gp_old.likelihood = ((self.gp_old)._reduced_likelihood_function((self.gp_old).optimal_theta))[0]
            self.gp_new.likelihood = ((self.gp_new)._reduced_likelihood_function((self.gp_new).optimal_theta))[0]
            difference = compute_percentage_difference(self.gp_old.likelihood,self.gp_new.likelihood)           
