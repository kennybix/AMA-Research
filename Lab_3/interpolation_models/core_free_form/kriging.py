...
class Kriging:
    ...
    def NLL(self, hyperparameter):
        ...
        np.fill_diagonal(R,(1.0+nugget)) #this should only be for the training samples
    ...
    def get_theta(self, theta0, nu0):
        ...
        if (self.optimizer=="CMA-ES"):
            ...
            # LB = [0.5] +  [1e-5]*len(self.theta0)
            # UB = [10] +  [1e6]*len(self.theta0)
            LB = [0.5] +  [1e-2]*len(self.theta0)
            UB = [10] +  [1e1]*len(self.theta0)
            ...
            res = minimize(self.NLL,xk,constraints=[{"fun": con, "type": "ineq"} for con in constraints],
                        method=self.optimizer,options={"rhobeg": _rhobeg, "tol": 1e-15, "maxiter": limit})
