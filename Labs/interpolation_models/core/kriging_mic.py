...
def estimate_MIC(x,y,decimal_place=4):
    w_MIC = []
    rows,columns = x.shape
    # loop through the data dimensions to compute the MIC values 
    for d in range(columns):
        mine = MINE(alpha=0.6, c=15, est="mic_approx")
        mine.compute_score(x[:,d], y.reshape(y.shape[0],))
...
class Kriging:
    ...
    def __init__(self,x,y,kernel,theta0="",optimizer="nelder-mead-c",optimizer_noise=1.0,eps=1.48e-08,restarts=1,preprocessing="standardize",n_comp="", pls_cv=5):
        ...
        self.coeff_mic = estimate_MIC(x.copy(),y.copy())
        # self.coeff_mic = [0.25]*20
    ...
    def compute_componentwise_distance(self,D,theta):
        theta_c = theta * np.array(self.coeff_mic)
        D_corr = np.einsum("j,ij->ij", (theta_c).T, D)
        return D_corr
    ...
    def get_theta(self,theta0):
        ...
        approximated_theta = theta * np.array(self.coeff_mic)
        self.info = {'Theta':self.theta,
                        'Approximated_theta': approximated_theta,
                        'MIC':self.coeff_mic,
