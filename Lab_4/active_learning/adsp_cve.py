...
def compute_cross_validation_score (data,k_folds="LOOCV"): #fix Gaussian kernel as default
    # shuffle the data based on the random state
    data['x'],data['y'] = shuffle(data['x'],data['y'])
    e_all = []
    if k_folds == "LOOCV": k_folds = (data['x']).shape[0]
    
    for k in range(k_folds):
        x_folds = np.array_split(data['x'],k_folds) 
        y_folds = np.array_split(data['y'],k_folds)

        x_test_data = x_folds.pop(k)  # get test data
        y_test_data = y_folds.pop(k)

        x_training_data = np.concatenate(x_folds) # get training_data
        y_training_data = np.concatenate(y_folds)
        
        # build model 
        model = KCORE2.Kriging(x_training_data,y_training_data,kernel="gaussian",optimizer="nelder-mead-c")
        model.train()
        # predict with model
        y_pred = (model.predict(x_test_data))[0]
        
        # evaluate model performance 
        e = mse(y_test_data,y_pred)
        e_all.append(e)
    cve = np.sqrt(np.mean(e_all))
    # cve = np.mean(e_all)
    return cve
...
class active_learn():
    ...
    def get_more_samples(self):
        ...
        # theta0 = [0.01]*self.dim # change back later
        theta0 = [0.5]*self.dim
        ...
        while(count<self.max_steps): #stopping criteria
            ...
            self.cve_old = compute_cross_validation_score({'x':x_sample,'y':y_sample}) # compute the first cve
            ...
            self.cve_new = compute_cross_validation_score({'x':x_sample,'y':y_sample}) # compute the first cve
            ...
            # self.gp_new = KCORE2.Kriging(x_sample,y_sample,self.kernel,theta0=theta0,optimizer="COBYLA") #initialiize model
            #replacing the nelder-mead-c optimizer with COBYLA to fix negative likelihoods
            # self.gp_new.train() # train new model
            difference = compute_percentage_difference(self.cve_old,self.cve_new)
            ...
            if (difference < self.stopping_criteria): 
        ...
        perc_name = str((self.obj_func).__name__)
