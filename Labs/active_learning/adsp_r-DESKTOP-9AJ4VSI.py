...
class active_learn():
    ...
    def loss_func(self,x):
        ...
        pos = ((self.x_data).tolist()).index(x_closest[0].tolist())       
        y = (self.x_data)[pos] # get the approximate value of y from the data
    ...
    def get_more_samples(self):
        ...
        while(count<self.max_steps): #stopping criteria
            ...
            x_new = self.closest(self.x_scaling_model.transform(self.x_data),self.x_scaling_model.transform(res['xopt'])) #get the similar value of x in the real dataset
            pos = ((self.x_data).tolist()).index(x_new[0].tolist())       
